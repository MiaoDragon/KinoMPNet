...
def main(args):
    ...
    def plan_one_path(obs_i, obs, obc, start_state, goal_state, goal_inform_state, max_iteration, data, out_queue):
        ...
        planner = vis_planners.DeepSMPWrapper(mlp_path, encoder_path, 
                                              ...
                                              20, num_steps, step_sz, propagate_system, args.device)
    ...
    for i in range(len(paths)):
        ...
        for j in range(len(paths[i])):
            ...
            state[-1] = paths[i][j][-1]
            data = state
            p = Process(target=plan_one_path, args=(obs_i, obs[i], obc[i], start_state, goal_state, goal_inform_state, 1000, data, queue))
            p.start()
            p.join()
            res = queue.get()
