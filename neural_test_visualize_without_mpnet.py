...
def main(args):
    ...
    for _ in range(T):
        ...
        if args.seen_N > 0:
            ...
            fes_path_, valid_path_ = eval_tasks(None, None, args.env_type, seen_test_data, args.model_path, 'seen', normalize_func, unnormalize_func, dynamics, jac_A, jac_B, enforce_bounds)
        ...
        if args.unseen_N > 0:
            ...
            fes_path_, valid_path_ = eval_tasks(None, None, args.env_type, unseen_test_data, args.model_path, 'unseen', normalize_func, unnormalize_func, dynamics, jac_A, jac_B, enforce_bounds)
            valid_path = valid_path_.flatten()
            fes_path = fes_path_.flatten()   # notice different environments are involved
