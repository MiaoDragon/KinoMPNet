...
def plan(obs, env, x0, xG, data, informer, init_informer, system, dynamics, enforce_bounds, IsInCollision, traj_opt, jac_A, jac_B, step_sz=0.02, MAX_LENGTH=1000):
    ...
    explored_states = []   # record all explored states, to avoid exploring same region multiple times
    explore_delta = 0.001
    ...
    while target_reached==0 and itr<MAX_LENGTH:
        ...
        if tree==0:
            ...
            if e is None:
                ...
                itr += 1
                continue
            # make sure the new node is not explored before
            explore_near = False
            for i in range(len(explored_states)):
                if node_nearby(explored_states[i], x.x, np.identity(len(x.x)), explore_delta, system):
                    explore_near = True
                    print('the new node is near the explored nodes')
                    break
            if explore_near:
                for_in_collision_nums[-1] += 1
                tree = 1
            ...
            # record this as explored
            explored_states.append(x0.x)
            
            for i in range(len(back_prev_scatter)):
                back_prev_scatter[i].remove()            
            back_prev_scatter = []
            xG = goal
            tree=1
        else:
            ...
            back_prev_scat = ax.scatter(xw.x[0], xw.x[1], c='red')
