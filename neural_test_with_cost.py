...
from iterative_with_cost.gem_eval import eval_tasks
...
from iterative_with_cost.data_structure import *
from iterative_with_cost.plan_general import propagate
...
def main(args):
    ...
    if args.start_epoch > 0:
        ...
        torch_seed, np_seed, py_seed = load_seed(os.path.join(model_folder, model_path))
        # set seed after loading
        torch.manual_seed(torch_seed)
        np.random.seed(np_seed)
        random.seed(py_seed)
