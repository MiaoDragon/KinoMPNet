...
def main(args):
    ...
    print(mlp_path)
    #####################################################
    def plan_one_path(obs_i, obs, obc, detailed_data_path, data_path, start_state, goal_state, goal_inform_state, cost_i, max_iteration, out_queue_t, out_queue_cost, random_seed, env_id, path_id):
        if args.env_type == 'pendulum':
            system = standard_cpp_systems.PSOPTPendulum()
            bvp_solver = _sst_module.PSOPTBVPWrapper(system, 2, 1, 0)
            step_sz = 0.002
        ...
        ax.scatter(xs_to_plot[:,0], xs_to_plot[:,2], c='lightgreen', s=0.5)
        # draw start and goal
        ...
        if solution is not None:
            ...
            ax.scatter(xs_to_plot[:,0], xs_to_plot[:,2], c='green', s=0.5)
            start_state_np = np.array(start_state)
            goal_state_np = np.array(goal_state)
            ax.scatter([start_state_np[0]], [start_state_np[2]], c='blue', marker='*')
            ax.scatter([goal_state_np[0]], [goal_state_np[2]], c='red', marker='*')
        ...
        if solution is not None:
            ...
            file = open('sst_path_env_%d_path_%d_path.pkl' % (env_id, path_id), 'wb')
            pickle.dump(res_x, file)
            file.close()
            file = open('sst_path_env_%d_path_%d_control.pkl' % (env_id, path_id), 'wb')
            pickle.dump(res_u, file)
            file.close()
            file = open('sst_path_env_%d_path_%d_cost.pkl' % (env_id, path_id), 'wb')
            pickle.dump(res_t, file)
            file.close()
    ...
    for i in range(len(paths)):
        ...
        # validation end
            ...
            for trial in range(1):
                ...
                #p = Process(target=plan_one_path, args=(obs_i, obs[i], obc[i], state, paths[i][j], start_state, goal_state, goal_inform_state, cost_i, 3000000, queue_t, queue_cost, random_seed))
                plan_one_path(obs_i, obs[i], obc[i], state, paths[i][j], start_state, goal_state, goal_inform_state, cost_i, 300000, queue_t, queue_cost, 0, i+args.seen_s, j+args.seen_sp)
                #p.start()
                #p.join()
                plan_t = queue_t.get()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--seen_N', type=int, default=2)
    parser.add_argument('--seen_NP', type=int, default=5)
