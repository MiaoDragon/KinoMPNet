...
def main(args):
    ...
    if args.env_name == 'pendulum':
    ...
    elif args.env_name == 'car_obs':
        env_constr = standard_cpp_systems.RectangleObs
        # randomly generate obstacle location
        obs_list = []
        width = 8.
        near = width * 2.0
        s_g_dis_threshold = 25 * 1.1
        goal_radius=2.0
        random_seed=0
        sst_delta_near=.6
        sst_delta_drain=0.2
        ...
        integration_step = 0.002
        print('generating obs...')
        obs_list, obc_list = car_obs_gen.obs_gen(args.N, args.N_obs, N_pc=2800, width=width)
        ## TODO: add other env
        # store the obstacles and obc first
        for i in range(len(obs_list)):
            file = open(args.path_folder+'obs_%d.pkl' % (i+args.s), 'wb')
    ...
    for i in range(args.N):
        ...
        if args.env_name == 'pendulum':
            ...
            env = env_constr(obs_list[i], width, 'car')
        ...
        for j in range(args.NP):
            ...
            while True:
                ...
                if args.env_name == 'pendulum':
                    ...
                    start, end = car_sg_gen.start_goal_gen(low, high, width, obs_list[i], obs_recs)
