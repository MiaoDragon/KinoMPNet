# generate new data continuing from previous ones
...
def main(args):
    ...
    print('loading obs and preivous paths...')
    obc, obs, path, sg, path_length, control, cost = data_loader.load_test_dataset(args.N, args.sp, args.path_folder, obs_f=True, s=args.s, sp=0)
    id_list = []
    for i in range(len(sg)):
        for j in range(len(sg[0])):
            id_new = str(round(np.sum(sg[i][j][0] + sg[i][j][1]), 2))
            if id_new not in id_list:
    ...
    for i in range(args.N):
        ...
        for j in range(args.NP):
            ...
            while True:
                ...
                # check if start and goal is in the previous list
                id_new = str(round(np.sum(start + end), 2))
