...
for obs_idx in range(0,2):
    for p_idx in range(1800,1800+5):
        ...
        #            [0., 3.],
        #            [10, -3.]]
        if os.path.exists('../cartpole_env%d_path%d_data.mp4' % (obs_idx, p_idx)):
            continue
        ...
        anim.save('../cartpole_env%d_path%d_data.mp4' % (obs_idx, p_idx), writer=writer)
        ...
        #            [0., 3.],
        #            [10, -3.]]
        if os.path.exists('../cartpole_env%d_path%d_sst.mp4' % (obs_idx, p_idx)):
            continue
        file = open('../data/cartpole_obs/obs_%d.pkl' % (obs_idx), 'rb')
        obs_list = pickle.load(file)
        file = open('../data/cartpole_obs/obc_%d.pkl' % (obs_idx), 'rb')
        ...
        print(obs_list.shape)
        # load path
        path = open('../sst_path_env_%d_path_%d_path.pkl' % (obs_idx, p_idx), 'rb')
        path = pickle.load(path)
        controls = open('../sst_path_env_%d_path_%d_control.pkl' % (obs_idx, p_idx), 'rb')
        controls = pickle.load(controls)
        costs = open('../sst_path_env_%d_path_%d_cost.pkl' % (obs_idx, p_idx), 'rb')
        costs = pickle.load(costs)
        params = {}
        params['pole_l'] = 2.5
        params['pole_w'] = 0.1
        ...
        anim = vis.animate(np.array(states), np.array(actions), np.array(costs), obs_list)
        #HTML(anim.to_html5_video())
        anim.save('../cartpole_env%d_path%d_sst.mp4' % (obs_idx, p_idx), writer=writer)
