...
def main(args):
    ...
    for envi in range(2):
        for pathi in range(10):
            ...
            print('length:', len(xs_to_plot))
            print('extracted: ', int(len(xs_to_plot)/args.num_steps))
            for k in range(50):
            ...
            for k in range(len(mpnet_dropout_paths)):
                ...
                for i in range(len(xs_to_plot)):
                    xs_to_plot[i] = wrap_angle(xs_to_plot[i], psopt_system)
                ax.scatter(xs_to_plot[:,0], xs_to_plot[:,2], c='lightgreen', alpha=0.5)
            ...
            ax.scatter(xs_to_plot_mean[:,0], xs_to_plot_mean[:,2], c='lightblue')
            ...
            for k in range(len(mpnet_dropout_paths)):
                ...
                for i in range(len(xs_to_plot)):
                    xs_to_plot[i] = wrap_angle(xs_to_plot[i], psopt_system)
                for i in range(len(xs_to_plot)):
                    ax.plot([xs_to_plot_mean[k,0], xs_to_plot[i,0]], [xs_to_plot_mean[k,2], xs_to_plot[i,2]], c='skyblue', alpha=0.5)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--seen_sp', type=int, default=802)
