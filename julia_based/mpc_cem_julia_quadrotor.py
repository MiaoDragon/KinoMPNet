import julia
from julia.api import Julia
import numpy as np
from time import time

if __name__ == '__main__':
    jl = Julia(compiled_modules=False)
    j = julia.Julia(runtime="julia")

    model = 'quadrotor'
    i_node = 2

    nt = 5

    mu_u = np.zeros((1, nt, 4))
    mu_u[:, :, 0] = 10
    sigma_u = np.ones((1, nt, 4))
    sigma_u[:, :, 0] *= 5
    mu_t, sigma_t = 0.05, 0.05

    cem = j.include("CEM_{}.jl".format(model))
    dx = j.include("{}.jl".format(model))
    q_base_path = '../../../../motion_planning/QuadrotorEnv/env1/path/'
    path = np.loadtxt(q_base_path+'path_25_27_74_36_25_33.txt')
    x = path[i_node]
    control, cost = [], []
    weights = np.ones(13)*0.1
    weights[:3] = 1
    weights[3:7] = 0.5

    start = path[i_node]
    goal = path[i_node+1]

    cem(start, start, weights, mu_u, sigma_u, mu_t, sigma_t, nt, False)
    for node in [i_node]:
        timer_start = time()
        for _ in range(30):
            u0, t0, mu_u, sigma_u, mu_t, sigma_t = \
                cem(start, goal, weights, mu_u, sigma_u, mu_t, sigma_t, nt, False)
            x = dx(x, u0, int(np.floor(t0/0.02))+1, 0.02)
            for para in [mu_u, sigma_u, mu_t, sigma_t]:
                para[:-1] = para[1:] 
            start = x.copy()
            loss = np.linalg.norm((np.array(start) - goal)*weights)
            cost.append((int(np.floor(t0/0.02))+1)*0.02)
            control.append(u0)
            print('loss:', loss)
            if(loss < 0.5):
                break
        timer_stop = time()
    print('cost:', cost, '\nsum', sum(cost))
    print('control:', control)
    print('time: ', timer_stop - timer_start)
